<script>
    ...
    function newGame() {
      ...
      setTimeout(() => {
        ...
        // Automatically place a bet of 10
        placeBet(10);

        // Run the basic strategy until the deck is empty
        let i = 0;
        let intervalId = setInterval(() => {
          if(deck.length > 8 && !gameOver) {
            basicStrategy();
          } else {
            clearInterval(intervalId);
          }
          i++;
        }, 1000); // Set delay of 1 second for each call of basicStrategy
      }, 250);
    }
